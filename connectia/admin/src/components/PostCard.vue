<template>
  <article class="pcard" :data-layout="layout" :class="{ pinned: post.pinned && show.pinned }">
    <header v-if="showHeader" class="pcard-head">
      ...
      <div class="meta">
        <p v-if="show.authorName" class="author">{{ post.authorName || 'Comunidad' }}</p>
        <p v-if="showSub" class="sub">
          <span v-if="show.tipo" class="tipo">{{ post.tipo }}</span>
          <span v-if="show.pinned && post.pinned"> · fijada</span>
          <span v-if="show.date && post.publishedAt"> · {{ formatPostDate(post.publishedAt) }}</span>
        </p>
      </div>
      <button
        v-if="showMoreMenu"
      ...
      <div v-if="show.media && hasMedia" class="media-wrap">
        ...
        >
          ...
          <div class="banner-overlay">
            <p v-if="show.tipo || (show.pinned && post.pinned)" class="tipo light">
              <template v-if="show.tipo">{{ post.tipo }}</template>
              <span v-if="show.pinned && post.pinned"> · fijada</span>
            </p>
    ...
    <template v-else>
      ...
      <div class="pcard-body">
        ...
        <p v-if="show.body && !compact" class="text" :class="{ clamp: truncate }">{{ post.cuerpo }}</p>
        <button
          v-if="show.body && truncate && post.cuerpo && post.cuerpo.length > 180"
          type="button"
          class="more"
          @click="emitOpen"
        >
...
const showHeader = computed(
  () =>
    ...
    (show.value.date && props.post?.publishedAt),
)

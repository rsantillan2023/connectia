...
const storage = multer.diskStorage({
  ...
  filename(_req, file, cb) {
    ...
    const name = `post-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${ext === '.jpeg' ? '.jpg' : ext}`
    cb(null, name)
  },
})
...
/**
 * Sube 1..N imágenes para publicaciones / carrusel.
 * multipart field: "files" (o "file")
 * Respuesta: { urls: ['/uploads/...'], url: '...' }
 */
router.post(
  '/',
  requireAuth,
  requireCapability('admin.publicaciones', 'admin.saludos'),
  (req, res, next) => {
